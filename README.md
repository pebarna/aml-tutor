...
You're going to build an AML/fraud triage system from scratch, in a sibling repo called
`aml-triage`, across three phases: a fraud-detection classifier (an XGBoost model trained on the
PaySim mobile-money dataset, with a time-based split, engineered features, class-imbalance
handling, and a chosen operating threshold backed by precision/recall/PR-AUC), a RAG-backed triage
agent that drafts an escalate/monitor/close recommendation with a cited rationale, and an eval
harness that measures both.
